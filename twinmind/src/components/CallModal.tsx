...
const CallModal: React.FC<CallModalProps> = ({ isOpen, contactName, avatarInitials, onEndCall }) => {
    ...
    // A hardcoded simulated transcript
    const transcriptLines = [
        `TwinMind AI: Recording and transcription started.`,
        `You: Hey ${contactName?.split(' ')[0] || contactName}, how is the new feature looking?`,
        `${contactName}: It is mostly done, but we really need to deploy those changes by EOD today.`,
        `You: Understood. TwinMind, please note the urgent deployment for EOD.`,
        `TwinMind AI: Noted. An urgent deployment is required by end of day.`,
    ]

    const handleEndCall = () => {
        // Generate MOM based on the static transcript provided
        const mom = `**MOM (Minutes of Meeting) with ${contactName}:**\n\n- Discussed latest features.\n- 🚨 **URGENT:** Deploy these changes by EOD.\n\n*(Transcript saved to OneDrive path: C:/Users/Srinivasa.p/OneDrive/Transcripts/Meeting_${new Date().toISOString().split('T')[0]}.docx)*\n\n*(Summary auto-generated by TwinMind AI)*`
        onEndCall(mom)
    }
                ...
                <div className="call-transcript-panel">
                    ...
                    <div className="transcript-scroll">
                        {transcriptLines.map((line, i) => (
                            <div key={i} className={`transcript-line ${i <= Math.floor(seconds / 2) ? 'visible' : 'hidden'}`}>
                                {line}
                            </div>
                        ))}
                        {(Math.floor(seconds / 2) < transcriptLines.length) && (
                            <div className="transcript-typing">...</div>
                        )}
                    </div>
                </div>
